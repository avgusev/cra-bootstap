...
export class PresetStore {
  ...
  loadPresets() {
    this.presets = JSON.parse(localStorage.getItem('savedFilters__accidents') || '[]');
  }

  savePresets() {
    localStorage.setItem('savedFilters__accidents', JSON.stringify(this.presets));
  }
  ...
  addPreset() {
    ...
    // localStorage.setItem('savedFilters__accidents', JSON.stringify(newPresets));
    this.presets.unshift({ name, filter: this.rootStore.accidentsStore.requestFilters });
    this.savePresets();
  ...
  activatePreset(name: string) {
    ...
    this.activePreset = name;
    this.rootStore.accidentsStore.stateFilters = preset.filter;
    this.rootStore.accidentsStore.loadFilter();
  }
  ...
  removeActivePreset(name?: string) {
    ...
    this.rootStore.accidentsStore.clearFilter();
  }
