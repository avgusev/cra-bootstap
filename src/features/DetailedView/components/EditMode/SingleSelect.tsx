...
function SingleSelect({ field }: SingleSelectProps) {
  ...
  const loadOptionsHandler = async (inputValue: string) => {
    const attributes = await fetchFieldAttributes(field.id);
    return attributes
      .map((el) => ({ label: el.text, value: el }))
      .filter((el) => (inputValue ? el.label?.indexOf(inputValue) !== -1 : true));
  };
